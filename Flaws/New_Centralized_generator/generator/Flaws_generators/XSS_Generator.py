                ...
                for s in ET.parse(self.fileManager.getXML("sanitize")).getroot():
                    ...
                    if "XSS" in sanitize.flaws:
                        for i in ET.parse(self.fileManager.getXML("input")).getroot():
                            ...
                            start = open(flaw.code[0], "r")
                            startcode = start.readlines()
                            for line in startcode:
                                file.addContent(line)

                            file.addContent(Input.code + "\n")
                            for code in sanitize.code:
                                file.addContent(code + "\n")

                            end = open(flaw.code[1], "r")
                            endcode = end.readlines()
                            for line in endcode :
                                file.addContent(line)
